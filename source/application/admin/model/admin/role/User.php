...
class User extends StoreUserModel
{
     ...
     * 更新记录
     * @param array $data
     * @param array $wxapp_id
     * @return bool
     * @throws \think\exception\DbException
     */
    public function edit($data,$wxapp_id)
    {
        ...
        try {
            ...
            $this->allowField(true)->save($data);
            // 更新角色关系记录
            (new UserRole)->edit($this['store_user_id'], $data['role_id'],$wxapp_id);
            $this->commit();
            return true;
        } catch (\Exception $e) {
            $this->error = $e->getMessage();
    ...
    public function getUid($data)
    {
        ...
        foreach ($data as $wxapp) {
            $names[$wxapp['wxapp_id']] = $this->where('wxapp_id','=',$wxapp['wxapp_id'])->field('store_user_id')->find()['store_user_id'];
        }
        return $names;
    }
}
